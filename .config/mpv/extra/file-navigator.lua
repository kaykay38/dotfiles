...
local settings = {
  ...
  favorites_file = os.getenv("HOME").."/.config/mpv/favorites.conf",
  favorites = {
  "/home/mia/OneDrive/CodeWorkspace/CSCD379-NETWebDev",
  "/home/mia/Videos",
  "/home/mia/Music",
    },
  ...
  mediaformats = {
    ...
    'h264','h265','hdmov','hdv','hes','hevc','kss','lpcm','m1a','m1v','m2a','m2t','m2ts','m2v','m3u','m3u8','m4a','m4v','mk3d','mka','mkv',
    'mlp','mod','mov','mp1','mp2','mp2v','mp3','mp4','mp4v','mp4v','mpa','mpe','mpeg','mpeg2','mpeg4','mpg','mpg4','mpv','mpv2','mts','mtv','mxf','nsf',
    'nsfe','nsv','nut','oga','ogg','ogm','ogv','ogx','opus','pcm','pls','qt','ra','ram','rm','rmvb','sap','snd','spc','spx','svg','thd','thd+ac3',
...
local file_lookup = {}
for _, ext in ipairs(settings.mediaformats) do
  file_lookup[ext] = true
end
...
function isvalidfile(file)
    local ext = file:match("^.+%.(.+)$")
    return ext and file_lookup[ext:lower()]
end
...
function scandirectory(searchdir)
  ...
  if popen then
    for direntry in popen:lines() do
      ...
      if isvalidfile(direntry) or direntry:find("^.+/$") and not matchedignore and not settings.ignorePaths[path..direntry] then
...
function cyclefavorite()
  local favorites_file = io.open(settings.favorites_file, "r")
  if not favorites_file then return nil end
  local favorites = {}
  for line in favorites_file:lines() do 
    table.insert(favorites,line)
  end
  local firstpath = favorites[1]
  if not firstpath then return end
  local favpath = nil
  local favlen = 0
  for key, fav in pairs(favorites) do
    favlen = favlen + 1
