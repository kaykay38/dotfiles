...
function NvimTreeXdgOpen()
  ...
  if node then
    vim.fn.jobstart("xdg-open '" .. node.absolute_path .. "' 2>/dev/null&", { detach = true })
  end
end
...
local tree_cb = require'nvim-tree.config'.nvim_tree_callback
    vim.g.nvim_tree_bindings = {
      ...
      ["-"]              = tree_cb("dir_up"),
      ["x"]              = ":lua NvimTreeXdgOpen()<CR>",
      ["o"]              = ":lua NvimTreeOpenWith()<CR>",
      -- ["x"]              = ':lua vim.fn.system("xdg-open "..require"nvim-tree.lib".get_node_at_cursor().absolute_path)<CR>',
      ["q"]              = tree_cb("close"),
    }
...
--    function()
--        vim.cmd"hi! NvimTreeGitIgnored guifg=gray"
--        vim.cmd"hi! NvimTreeGitIgnoredFolder guifg=gray"
--    end
--)
